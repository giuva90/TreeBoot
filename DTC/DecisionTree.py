...
def cleanup_csv(line):
	...
	for item in white_spaced:
		...
		line = str.replace(line, "," + item, "," + litem) if line.endswith(item) else str.replace(line,
		                                                                                          "," + item + ",",
		                                                                                          "," + litem + ",")
...
class DecisionTree(object):
	...
	def get_training_data(self):
		...
		class_for_sample_dict = {"sample_" + key:
			                         class_column_heading + "=" + data_dict[key][self._csv_class_column_index - 1] for
		                         key in data_dict}
		sample_names = ["sample_" + key for key in data_dict]
		feature_values_for_samples_dict = {"sample_" + key:
			                                   list(map(operator.add, list(
				                                   map(operator.add, feature_names, "=" * len(feature_names))),
			                                            [str(convert(data_dict[key][i - 1])) for i in
			                                             self._csv_columns_for_features]))
		                                   for key in data_dict}
		features_and_values_dict = {all_feature_names[i - 1]:
			                            [convert(data_dict[key][i - 1]) for key in data_dict] for i in
		                            self._csv_columns_for_features}
	...
	def show_training_data(self):
		...
		for feature in sorted(features):
			nums, strings = [x for x in self._features_and_values_dict[feature] if isinstance(x, (float, int))], [x for
			                                                                                                      x in
			                                                                                                      self._features_and_values_dict[
				                                                                                                      feature]
			                                                                                                      if
	...
	def recursive_descent(self, node):
		...
		if self._debug3:
			...
			print("\nRD10 Calculated entropies for different values of best feature: %s" % str(
				best_feature_val_entropies))
			...
			if best_feature in self._numeric_features_valuerange_dict and \
				...
				for feature_and_value_index in range(len(feature_value_combos)):
					...
					class_probabilities = list(map(lambda x:
					                               self.probability_of_a_class_given_sequence_of_features_and_values_or_thresholds(
						                               x, extended_branch_features_and_values_or_thresholds),
					                               self._class_names))
			...
			if feature_name in symbolic_features_already_used:
				...
				for value in newvalues:
					...
					partitioning_entropy = entropy1 * \
					                       self.probability_of_a_sequence_of_features_and_values_or_thresholds(
						                       for_left_child) \
					                       + entropy2 * \
					                         self.probability_of_a_sequence_of_features_and_values_or_thresholds(
						                         for_right_child)
	...
	def class_entropy_for_less_than_threshold_for_feature(self, \
	                                                      array_of_features_and_values_or_thresholds, feature,
	                                                      threshold):
	...
	def probability_of_feature_value(self, feature_name, value):
		...
		if feature_name in self._numeric_features_valuerange_dict:
			if self._feature_values_how_many_uniques_dict[
				feature_name] > self._symbolic_to_numeric_cardinality_threshold:
				if feature_name not in self._sampling_points_for_numeric_feature_dict:
					...
					sampling_points_for_feature = [valuerange[0] + histogram_delta * j for j in
					                               range(num_of_histogram_bins)]
	...
	def probability_of_feature_value_given_class(self, feature_name, feature_value, class_name):
		...
		if feature_name in self._numeric_features_valuerange_dict:
			if self._feature_values_how_many_uniques_dict[
				...
				values_for_feature = list(set(self._features_and_values_dict[feature_name]))
				values_for_feature = list(filter(lambda x: x != 'NA', values_for_feature))
				values_for_feature = list(
					map(lambda x: "".join([feature_name, "=", x]), map(str, map(convert, values_for_feature))))
				value_counts = [0] * len(values_for_feature)
				for sample in samples_for_class:
					features_and_values = self._training_data_dict[sample]
					for i in range(0, len(values_for_feature)):
				...
				if total_count == 0:
					raise Exception(
						'''PFVC2 Something is wrong with your training file. It contains no training samples '''
	...
	def probability_of_feature_less_than_threshold_given_class(self, feature_name, threshold, class_name):
		...
		probability = ((1.0 * len(actual_points_for_feature_less_than_threshold)) / len(
			actual_feature_values_for_samples_in_class)) if len(actual_feature_values_for_samples_in_class) > 0 else 0.0
		...
		for feature_name in true_numeric_types_feature_names:
			if lowerbound[feature_name] is not None and upperbound[feature_name] is not None \
				...
				if not probability:
					...
					probability *= (
					self.probability_of_feature_less_than_threshold(feature_name, upperbound[feature_name]) - \
	...
	def display_decision_tree(self, offset):
		...
		if len(self.get_children()) > 0:
			...
			print_class_probabilities_with_class = [self.get_class_names()[i] + " => " +
			                                        print_class_probabilities[i] for i in
			                                        range(len(self.get_class_names()))]
	...
	def evaluate_training_data(self):
		...
		for fold_index in range(10):
			...
			trainingDT._features_and_unique_values_dict = {feature:
				                                               sorted(list(
					                                               set(trainingDT._features_and_values_dict[feature])))
			                                               for
			...
			trainingDT._numeric_features_valuerange_dict = {feature:
				                                                [min(trainingDT._features_and_unique_values_dict[
					                                                     feature]),
				                                                 max(trainingDT._features_and_unique_values_dict[
					                                                     feature])]
			                                                for feature in self._numeric_features_valuerange_dict}
			if evaldebug:
			...
			trainingDT._feature_values_how_many_uniques_dict = {feature: []
			                                                    for feature in self._features_and_unique_values_dict}
			trainingDT._feature_values_how_many_uniques_dict = {feature:
				                                                    len(trainingDT._features_and_unique_values_dict[
					                                                        feature])
			                                                    for feature in self._features_and_unique_values_dict}
	...
	def read_parameter_file_symbolic(self):
		...
		for item in features:
			...
			for i in range(0, len(splits)):
				if i == 0:
					features_and_values_dict[splits[0]] = []
				else:
...
class DTIntrospection(object):
	...
	def recursive_descent(self, node):
		...
		if self._debug:
			print("\nat node " + str(node_serial_number) + ":  the branch features and values are: " + str(
	...
	def recursive_descent_for_showing_samples_at_a_node(self, node):
		...
		if node_serial_number in self._samples_at_nodes_dict:
			if self._debug:
				print("\nat node " + str(node_serial_number) + ":  the branch features and values are: " + str(
					branch_features_and_values_or_thresholds))
			print("Node " + str(node_serial_number) + ": the samples are: " + str(
				self._samples_at_nodes_dict[node_serial_number]))
		children = node.get_children()
	...
	def display_training_samples_to_nodes_influence_propagation(self):
		for sample in sorted(self._dt._training_data_dict, key=lambda x: sample_index(x)):
			if sample in self._sample_to_node_mapping_direct_dict:
				...
				list(map(
					lambda x: self.recursive_descent_for_sample_to_node_influence(x, nodes_directly_affected, "    "),
					nodes_directly_affected))
	...
	def recursive_descent_for_sample_to_node_influence(self, node_serial_num, nodes_already_accounted_for, offset):
		...
		list(map(lambda x: self.recursive_descent_for_sample_to_node_influence(x, children_affected, offset),
		         children_affected))
	...
	def explain_classification_at_one_node(self, node_id):
		...
		if (ans == 'y') or (ans == 'yes'):
			...
			for cls in class_names:
				prob = self._dt.probability_of_a_class_given_sequence_of_features_and_values_or_thresholds(cls,
				                                                                                           sequence)
				print("\n    Probability of class %s given just one feature test %s is: %s" % (
				cls, str(sequence), str(prob)))
		else:
			print("goodbye")
